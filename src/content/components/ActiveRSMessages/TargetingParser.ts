...
export function getTargetingAttributes(expr: String): ParsedTargetingExpression {
    const tokens: Array<Token> = parseTargetingExpression(expr);
    const targetingTokens: ParsedTargetingExpression = { version: [], locale: []};
    for (let i = 0; i < tokens.length; i++) {
        if (TOKENS_WHITELIST.has(tokens[i].value)) {
            ...
            switch (tokens[i + 2].type) {
                ...
                default: value = `${value} ${tokens[i + 2].value}`
            }
