    ...
    return (
      <tbody>
        {bugs.map(({ id, displayName, status }) => (
          <tr
            className={isBugResolved({ status }) ? styles.resolved : ""}
            key={id}>
            <td className={styles.idColumn}>
              <a
                target="_blank"
                href={OPEN_BUG_URL + id}
                rel="noopener noreferrer">
                {id}
              </a>
            </td>
    ...
    return (
      <div className={styles.container}>
        ...
        <p className={styles.subheading}>
          Note: To add a feature to sidebar, set the priority of the meta bug to{" "}
          <strong>P1</strong>.<br />
          Setting your feature to <strong>P1</strong> will prioritize it for the
          current release and <strong>P2</strong> for the next.
        </p>
        {!empty ? (
          <table className={styles.featureTable}>
            {this.renderTableHead(
              "Meta Bug",
              `Prioritized for Firefox ${release}`
            )}
