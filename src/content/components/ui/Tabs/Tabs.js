...
export class Tabs extends React.PureComponent {
  ...
  renderTabLink(tabInfo, i) {
    return (<li key={i}>
      <NavLink exact={true} activeClassName={styles.activeTab} to={this.props.baseUrl + tabInfo.path}>
        {tabInfo.label}
      </NavLink>
    </li>);
  }
  ...
  renderTabRoute(config, i) {
    return (<Route
      ...
      render={config.render} />); // eslint-disable-line
  }
  ...
  render() {
    ...
    return (<div>
      ...
      <Switch>
        {config.map(this.renderTabRoute)}
      </Switch>
    </div>);
  }
}
