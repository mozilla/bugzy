 ...
 * @example $ new IterationLookup(["101.1 - Jan 4 - 15", "101.2 - Jan 18 - 29"])
 *          > {
 *              byDate: { "2019-01-07": "101.1", "2019-01-14": "101.1", ... },
 *              byVersionString: { "101.1": { startDate: "2019-01-04", ... } },
 *              orderedVersionStrings: ["101.1", "101.2"], ...
 *            }
 */
export class IterationLookup {
  constructor(iterations: string[]) {
    this.byDate = {};
    this.byVersionString = {};
    this.orderedVersionStrings = [];
    ...
    for (const [iteration, range] of rangesByIteration) {
      ...
      if (match) {
        ...
        if (startDateTime && endDateTime && weeks) {
          this.byVersionString[iteration] = {
            startDate: startDateTime,
            weeks,
            endDate: endDateTime,
          };
          this.orderedVersionStrings.push(iteration);
          const start = DateTime.fromISO(startDateTime);
          for (let i = 0; i < weeks; i++) {
            const monday = start.plus({ weeks: i });
            this.byDate[monday.toFormat("yyyy-MM-dd")] = iteration;
          }
        }
      }
    }
...
export class Iterations implements IterationLookup {
  ...
  orderedVersionStrings: string[];
  // Utility to get the number of work days between two dates
  getWorkDays: (startDate: Date | string, endDate: Date | string) => number;
  // Utility to get the Monday before a given date
  getMondayBefore: (date: DateTime) => DateTime;

  constructor(iterationLookup: IterationLookup) {
    this.byVersionString = iterationLookup.byVersionString;
    this.orderedVersionStrings = iterationLookup.orderedVersionStrings;
    this.byDate = iterationLookup.byDate;
    this.getWorkDays = Iterations.getWorkDays.bind(this);
    this.getMondayBefore = Iterations.getMondayBefore.bind(this);
    this.getIteration = this.getIteration.bind(this);
    this.getAdjacentIteration = this.getAdjacentIteration.bind(this);
  }
  ...
  getIteration(dateString?: string | DateTime): LegacyIteration {
    ...
    const date =
      typeof dateString === "string"
        ...
        : dateString;
    const monday = Iterations.getMondayBefore(date);
   ...
   * @returns {number}
   */
  static getWorkDays(startDate: Date | string, endDate: Date | string): number {
   ...
   * @returns {DateTime}
   */
  static getMondayBefore(date: DateTime | Date | string): DateTime {
